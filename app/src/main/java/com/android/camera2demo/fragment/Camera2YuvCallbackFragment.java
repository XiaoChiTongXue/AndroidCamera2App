...
//                        .show(getChildFragmentManager(), FRAGMENT_DIALOG);
    ...
    private void printSize(Size[] sizes){
        for(Size size:sizes){
            Log.v(TAG,""+size.getWidth() + " * " + size.getHeight() + "\n");
        }
    }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                Log.v(TAG,"---- rotatedPreviewWidth:"+rotatedPreviewWidth + ";rotatedPreviewHeight: "+rotatedPreviewHeight + "\n"
                        + ";rotatedPreviewHeight: " + rotatedPreviewHeight + "\n"
                        +  "; maxPreviewWidth："+maxPreviewWidth + "\n"
                        + ";maxPreviewHeight:"+maxPreviewHeight + "\n"
                        + ";largest size:"+largest.getWidth() + " * "+ largest.getHeight() + "\n");
...
//                        maxPreviewHeight, largest);
     ...
     */
    private static class YuvSaver implements Runnable {
        /**
         * The file we save the image into.
         */
        private final File mFile;
        private final byte[] mYByte;
        private final byte[] mUByte;
        private final byte[] mVByte;
        private final int mStride;
        private final int mHeight;
        private byte[] nv21;

        YuvSaver(byte[] yByte, byte[] uByte, byte[] vByte, int stride, int imageHeight,File file) {
            mYByte = yByte;
            mUByte = uByte;
            mVByte = vByte;
            mStride = stride;
        ...
        public void run() {
            if (nv21 == null) {
                nv21 = new byte[mStride * mHeight * 3 / 2];
            }

            if (nv21 != null && (nv21.length != mStride * mHeight * 3 / 2)) {
                return;
            }

            // 回传数据是YUV422
            if (mYByte.length / mUByte.length == 2) {
                ImageUtil.yuv422ToYuv420sp(mYByte, mUByte,mVByte, nv21, mStride, mHeight);
            }
            // 回传数据是YUV420
            else if (mYByte.length / mUByte.length == 4) {
                ImageUtil.yuv420ToYuv420sp(mYByte, mUByte,mVByte, nv21, mStride, mHeight);
            }
            ...
            Log.v(TAG,"save yuv file success:" + mFile.getAbsolutePath());
        }
    }
